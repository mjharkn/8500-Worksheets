...
```{r}
paste("Good Morning", "West Virginians")
```

(@) Can you explain what happened in 2-3 sentences? 

> The paste function is taking the two character vectors of "Good Morning" and "West Virginians" and combining them. As a result, when the cell is ran in R, it pastes the two vectors as one sentence rather than separating them as they are in the parentheses.
...
> 5 does not equal 10, so the return value is FALSE
> 3 is less than 4, so the return value is TRUE
> 5 does not equal 10, so the return value is TRUE
> 5 is equal to 5, and since the logical operator states that 5 does not equal 5, the return value is FALSE
> 3 is equal to 4 OR 3 (in this case, 3), so the return value is TRUE
> 3 does not equal 4 and 3 does not equal 5, so the return value is TRUE
...
> The vector is being multiplied by 2, it is multiplying every number in the vector by 2 and spitting out the results. So, 2 * 2 is 4, 4 * 2 is 8, 6 * 2 is 12, and 8 * 2 is 16. It is giving us all of the results separated by spaces since the code doesn't specify which number assigned to the vector to multiply.
...
> The line of code is saying to multiply the third number assigned to the vector by 2. Since the third number assigned to the vector is 6, it is multiplying 6 by 2, hence why it gives us the answer of 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my.cats <- c("Nermal", "Pebbles", "Stella")
```
...
> According to the documentation, the 'floor()' function returns the largest integer value less than or equal to the specified number.
...
> In row z, x = 1:5, so the five rows of sequential values that we see in row x are first being multiplied by 2. Then, they are getting 1 added on to them, since y = 1.
...
> The documentation tells us that the dataset contains information from Bob Damron's Address Books which span the years of 1965 through 1985.
...
> I figured out both of these prompts by typing ?head() into the console and reading the documentation. To figure out how to see the first 10 rows of data, I looked at the syntax given in the documentation and saw that n was an argument used to display the desired number of lines in a dataset. The documentation also covers tails(), which I realized was how you get the last 6 rows of data.
...
> "gayguides" is pulling data from the dataset, "$" is acting as an operator that lets us see a segment of the data, "title" is the segment of data we want the operator to allow us to see, and "500" is the specific item from the data segment that we want to see. So, when this code is ran the output of "Rest Stop" is the title of the 500th item of the dataset.
...
min(gayguides$Year)
```
- 1965 is the earliest year.

(@) How about the latest year? 
```{r}
max(gayguides$Year)
```
- 1985 is the latest year.
...
```{r}
head(BostonWomenVoters$Age, 500)
tail(BostonWomenVoters$Age, 500)
```

```{r}
head(BostonWomenVoters$Occupation, 500)
tail(BostonWomenVoters$Occupation, 500)
```
(@) What did you learn about the dataset? What can you tell me about it? 

>
BostonWomenVoters contains data on women who registered to vote after the passing of the 19th amendment in 1920. The info on includes the women's names, ages, address/location, placce of birth, occupation, closest male relative (husband, father), etc.

The importance of including the closest male relative and all of the data on him (naturalization info, address, etc.) tells me that it was integral for the federal government to have a male relative connected to the woman in order to confirm/validate her right to vote; she cannot be totally independent of a male.

Furthermore, the dataset seems to be sorted by age, with the youngest registered voters being 20 and the oldest being 91. This tells me that women of all ages were eager to vote. Looking at the occupations also tells us common occupations for women in the 1920s. Women in their 20s/30s seemed to be more of a mixed bag—-there are many clerks, stenographers, bookeepers, nurses, teachers, housewives, and some students. Looking at the occupations for the older women, there are many more housewives, housekeepers, and lots of widows. This gives us insight into the job types that most women kept outside of the home.

Overall, this dataset on newly registered women voters in Boston is historically valuable because it gives insight into not only the important aspects of women's voter registration (such as the attachment to a male relative), but also more general information on women's lives, such as their occupation, and how it aligns with their age.
